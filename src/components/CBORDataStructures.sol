...
library CBORDataStructures {
        ...
        returns (bytes[2][] memory decodedMapping)
    {
        ...
        for (uint256 item = 0; item < totalItems; item++) {
            ...
            (Spec.MajorType majorType, uint8 fieldShortCount, uint256 start, uint256 end, uint256 next) =
                Utils.parseField(encoding, mappingCursor);

            // Save our data
            decodedMapping[pair][arrayIdx] = Utils.extractValue(encoding, majorType, fieldShortCount, start, end);
        ...
        returns (bytes[] memory decodedArray)
    {
        ...
        for (uint256 item = 0; item < totalItems; item++) {
            // See what our field looks like
            (Spec.MajorType majorType, uint8 fieldShortCount, uint256 start, uint256 end, uint256 next) =
                Utils.parseField(encoding, arrayCursor);

            // Save our data
            decodedArray[item] = Utils.extractValue(encoding, majorType, fieldShortCount, start, end);
     ...
     */
    function parseDataStructure(bytes memory encoding, uint256 cursor, Spec.MajorType majorType, uint256 shortCount)
        internal
        view
    ...
    function getDataStructureItemLength(
        ...
        totalItems = ByteUtils.bytesToUint256(ByteUtils.sliceBytesMemory(encoding, countStart, countEnd));
